    ...
    <div style={{width: "100%", display: "flex", justifyContent: "center", flexDirection: "column", alignItems: "center"}}>
      ...
      <div style={{width: "100%", maxWidth: 1024, display: "flex", flexDirection: "row", flexWrap: "wrap", gap: 10, padding: 20, boxSizing: "border-box", background: "#f7f9fa", borderRadius: 7,boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px", backgroundColor: "#fff"}}>
        <div className='sdfadfsasdaas' style={{width: "49%"}}>
          ...
          <input type="text" style={{padding: 10, borderRadius: 6, width: "90%", border: "1px solid #cdd0d1"}} />
        ...
        <>
          {
            ...
            Array.from(Array(parseInt(location.state.ps.split(".")[0])).keys()).map((item, key)=> <FormCustomer title={"Người lớn "+(parseInt(key) + 1)} key={key} {...item} />)
          }
            
          {
            Array.from(Array(parseInt(location.state.ps.split(".")[1])).keys()).map((item, key)=> <FormCustomer title={"Trẻ em "+ (parseInt(key) + 1)} {...item} key={key} />)
          }
          {
            Array.from(Array(parseInt(location.state.ps.split(".")[2])).keys()).map((item, key)=> <FormCustomer title={"Em bé "+ (parseInt(key) + 1)} {...item} key={key} />)
          }
        </>
      }
      {
      ...
      <div style={{width: "100%", maxWidth: 1024, display: "flex"}}>
        <div className='sdjsiajwiawa' style={{padding: "8px 40px", background: "#2e89ff", color: "#fff", fontWeight: 600, borderRadius: 8, width: "max-content", cursor: "pointer"}}>
          Chọn
        </div>
      </div>
      <br />
